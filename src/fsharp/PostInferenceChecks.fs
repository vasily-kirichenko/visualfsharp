...
let rec CheckTypeDeep (cenv: cenv) ((visitTy,visitTyconRefOpt,visitAppTyOpt,visitTraitSolutionOpt, visitTyarOpt) as f) g env isInner ty =
...
let CheckTypeForAccess (cenv:cenv) env objName valAcc m ty =
    if cenv.reportErrors then 
        ...
        CheckTypeDeep cenv (visitTye, None, None, None, None) cenv.g env false ty
    ...
    if cenv.reportErrors then 
        ...
        let visitAppTy (tcref,tinst) = 
            if isByrefLikeTyconRef cenv.g m tcref then
                ...
                CheckTypesDeep cenv (visitTye, None, None, None, None) cenv.g env tinst
