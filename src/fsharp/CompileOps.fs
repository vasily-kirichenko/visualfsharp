...
type IProjectReference = 
    ...
    abstract EvaluateRawContents : System.Threading.CancellationToken -> IRawFSharpAssemblyData option
    /// Get the logical timestamp that would be the timestamp of the assembly file generated by the project
    abstract GetLogicalTimeStamp : System.Threading.CancellationToken -> System.DateTime option
...
type AssemblyResolution = 
    ...
    member this.ILAssemblyRef(ct: System.Threading.CancellationToken) = 
        ...
        | None ->
            let assRefOpt = 
                ...
                | Some r ->   
                    match r.EvaluateRawContents(ct) with 
                    | None -> None
                    | Some contents -> 
...
type TcAssemblyResolutions(results : AssemblyResolution list, unresolved : UnresolvedAssemblyReference list) = 
    ...
    member tcResolution.TryFindByExactILAssemblyRef ct assref = results |> List.tryFind (fun ar -> ar.ILAssemblyRef ct = assref)
...
type TcImports(tcConfigP:TcConfigProvider, initialResolutions:TcAssemblyResolutions, importsBase:TcImports option, ilGlobalsOpt) = 
    ...
    member tcImports.TryFindDllInfo (ctok: CompilationThreadToken, m, assemblyName, lookupOnly, ct) =
        CheckDisposed()
        ...
        | Some res -> Some res
        | None ->
            tcImports.ImplicitLoadIfAllowed(ctok, m, assemblyName, lookupOnly, ct)
            look tcImports
    

    member tcImports.FindDllInfo (ctok, m, assemblyName, ct) =
        match tcImports.TryFindDllInfo (ctok, m, assemblyName, lookupOnly=false, ct = ct) with 
        | Some res -> res
    ...
    member tcImports.FindCcuInfo (ctok, m, assemblyName, lookupOnly, ct) = 
        CheckDisposed()
        let rec look (t:TcImports) = 
    ...
    member tcImports.FindCcu (ctok, m, assemblyName,lookupOnly, ct) = 
        CheckDisposed()
        match tcImports.FindCcuInfo(ctok, m, assemblyName, lookupOnly, ct) with
    ...
    member tcImports.FindCcuFromAssemblyRef(ctok, m, assref:ILAssemblyRef, ct) = 
        CheckDisposed()
        match tcImports.FindCcuInfo(ctok, m, assref.Name, lookupOnly=false, ct = ct) with
        | ResolvedImportedAssembly(importedAssembly) -> ResolvedCcu(importedAssembly.FSharpViewOfMetadata)
        | UnresolvedImportedAssembly _ -> UnresolvedCcu(assref.QualifiedName)


#if EXTENSIONTYPING
    member tcImports.GetProvidedAssemblyInfo(ctok, m, assembly: Tainted<ProvidedAssembly>, ct) = 
        ...
        match tcImports.FindCcu (ctok, m, ilShortAssemName, lookupOnly=true, ct = ct) with 
        | ResolvedCcu ccu -> 
            if ccu.IsProviderGenerated then 
                let dllinfo = tcImports.FindDllInfo(ctok,m,ilShortAssemName,ct)
        ...
        | UnresolvedCcu _ -> 
            ...
            let ccuData : CcuData = 
              { IsFSharp=false
                ...
                ImportProvidedType = (fun ty -> Import.ImportProvidedType (tcImports.GetImportMap ct) m ty)
    ...
    member tcImports.GetImportMap(ct: CancellationToken) = 
        CheckDisposed()
        let loaderInterface = 
            { new Import.AssemblyLoader with 
                 member x.FindCcuFromAssemblyRef (ctok, m, ilAssemblyRef) = 
                     tcImports.FindCcuFromAssemblyRef (ctok, m,ilAssemblyRef, ct)
#if EXTENSIONTYPING
                 member x.GetProvidedAssemblyInfo (ctok, m, assembly) = tcImports.GetProvidedAssemblyInfo (ctok, m, assembly, ct)
    ...
    member private tcImports.InjectProvidedNamespaceOrTypeIntoEntity 
        ...
        | next::rest ->
            ...
            | Some childEntity ->
                tcImports.InjectProvidedNamespaceOrTypeIntoEntity (typeProviderEnvironment, tcConfig, m, childEntity, next::injectedNamspace, rest, provider, st, ct)
            | None -> 
                ...
                tcImports.InjectProvidedNamespaceOrTypeIntoEntity (typeProviderEnvironment, tcConfig, m, newNamespace, next::injectedNamspace, rest, provider, st, ct)
        | [] -> 
            match st with
            | Some st ->
                ...
                let importProvidedType t = Import.ImportProvidedType (tcImports.GetImportMap ct) m t
    ...
    member tcImports.ImportTypeProviderExtensions 
               (ctok, tcConfig:TcConfig, 
                ...
                entityToInjectInto, invalidateCcu:Event<_>, m, ct: CancellationToken) = 
                ...
                for provider in providers do 
                    try
                        ...
                        let rec loop (providedNamespace: Tainted<IProvidedNamespace>) =
                            ...
                            tcImports.InjectProvidedNamespaceOrTypeIntoEntity (typeProviderEnvironment, tcConfig, m, entityToInjectInto, [],path, provider, None, ct)
                            ...
                            for st in ptys do 
                                tcImports.InjectProvidedNamespaceOrTypeIntoEntity (typeProviderEnvironment, tcConfig, m, entityToInjectInto, [], path, provider, Some st, ct)
    ...
    member tcImports.PrepareToImportReferencedILAssembly (ctok, m, filename, dllinfo:ImportedBinary, ct) =
        CheckDisposed()
        let tcConfig = tcConfigP.Get(ctok)
        ...
        let ccu = Import.ImportILAssembly(tcImports.GetImportMap,m,auxModuleLoader,ilScopeRef,tcConfig.implicitIncludeDir, Some filename,ilModule,invalidateCcu.Publish,ct)
        ...
        let phase2 () = 
#if EXTENSIONTYPING
            ccuinfo.TypeProviders <- tcImports.ImportTypeProviderExtensions (ctok, tcConfig, filename, ilScopeRef, ilModule.ManifestOfAssembly.CustomAttrs.AsList, ccu.Contents, invalidateCcu, m, ct)
#endif
            [ResolvedImportedAssembly(ccuinfo)]
        phase2

    member tcImports.PrepareToImportReferencedFSharpAssembly (ctok, m, filename, dllinfo:ImportedBinary,ct) =
        CheckDisposed()
        let tcConfig = tcConfigP.Get(ctok)
        tcConfig.CheckFSharpBinary (filename, dllinfo.ILAssemblyRefs, m)
                    ...
                    lazy 
                        (match Map.tryFind ccuName optDatas  with 
                         ...
                         | Some info -> 
                            ...
                            let res = data.OptionalFixup(fun nm -> availableToOptionalCcu(tcImports.FindCcu(ctok,m,nm,lookupOnly=false,ct=ct))) 
        ...
        let phase2 () = 
            ...
            ccuRawDataAndInfos |> List.iter (fun (data,_,_) -> data.OptionalFixup(fun nm -> availableToOptionalCcu(tcImports.FindCcu(ctok,m,nm,lookupOnly=false,ct=ct))) |> ignore)
#if EXTENSIONTYPING
    ...
    member tcImports.RegisterAndPrepareToImportReferencedDll (ctok, r:AssemblyResolution, ct) : _ * (unit -> AvailableImportedAssembly list)=
        CheckDisposed()
        let m = r.originalReference.Range
        let filename = r.resolvedPath
        let contentsOpt = 
            match r.ProjectReference with 
            | Some ilb -> ilb.EvaluateRawContents ct
            | None -> None
        ...
        if tcImports.IsAlreadyRegistered ilShortAssemName then 
            let dllinfo = tcImports.FindDllInfo(ctok,m,ilShortAssemName,ct)
            let phase2() = [tcImports.FindCcuInfo(ctok,m,ilShortAssemName,lookupOnly=true,ct=ct)] 
            dllinfo,phase2
        else 
            let dllinfo = 
    ...
    member tcImports.RegisterAndImportReferencedAssemblies (ctok, nms:AssemblyResolution list, ct) =
        CheckDisposed()
        ...
        let dllinfos,phase2s = 
           nms |> List.choose 
                    (fun nm ->
                        try
                            Some(tcImports.RegisterAndPrepareToImportReferencedDll (ctok, nm, ct))
                        with
    ...
    member tcImports.DoRegisterAndImportReferencedAssemblies(ctok, nms, ct) = 
        CheckDisposed()
        tcImports.RegisterAndImportReferencedAssemblies(ctok, nms, ct) |> ignore

    member tcImports.ImplicitLoadIfAllowed (ctok, m, assemblyName, lookupOnly, ct: CancellationToken) = 
        CheckDisposed()
        ...
        if not lookupOnly && tcConfig.implicitlyResolveAssemblies then 
            let tryFile speculativeFileName = 
                ...
                | OkResult (warns, res) ->
                    ...
                    tcImports.DoRegisterAndImportReferencedAssemblies(ctok, res, ct)
                    true
